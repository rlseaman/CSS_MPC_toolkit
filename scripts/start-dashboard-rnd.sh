...
# and with --rnd + --dev-tabs:
#   --rnd      enables R&D surfaces (also on prod now that NEO
#              Consensus has shipped).
#   --dev-tabs gates in-flight tabs that aren't ready for prod
#              (Station Report Phase 1 currently). Prod's
#              start-dashboard.sh deliberately omits this flag.
# Cloudflare's named tunnel routes `dev.hotwireduniverse.org` to
# localhost:8051; access is gated by a Cloudflare Access policy.
#
...
echo "PGHOST=$PGHOST  port=8051  rnd=true  dev-tabs=true  waitress=true" >>"$LOG"
...
exec ./venv/bin/python app/discovery_stats.py --rnd --dev-tabs \
    --port 8051 --waitress >>"$LOG" 2>&1
