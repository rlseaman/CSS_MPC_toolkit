...
| **NEO Consensus** | Cross-source membership view of the `css_neo_consensus` schema (MPC NEA.txt, mpc_orbits, JPL CNEOS, ESA NEOCC, CSS NEOfixer, Lowell astorb). Per-source 3-state radios; class / numbered / named; numeric ranges (q/e/i/H/U/Nopp) + date ranges (first/last obs); "Misc. filters" column with Alias state, `Disc by` obscode, 7-bin Arc (sub-day bins down to `< 3 h`) + precise Arc-hr range, Sites n_stns 3-state. `Find:` search box + `Open external links in:` dropdown drive a per-row `↗` markdown link to JPL SBDB / Sentry / MPC Explorer / MPC DB / NEOfixer / ESA NEOCC. Disagreement-breakdown bar + UpSet plot with overlay color radio (Pattern / Site / H size / Decade / Arc / q distance), Show Mixed + Show Rare bins, height S/M/L, click-to-filter on overlay segments. Histograms strip + station × pattern heatmap below the table. Downloads support packed format. |
