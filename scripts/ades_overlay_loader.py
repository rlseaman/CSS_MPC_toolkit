...
def main():
    ...
    tracklets, nf, nobs = parse_batch(a.archive, a.max_files)
    print(f"  files={nf}  observations={nobs:,}  reprocessed tracklets={len(tracklets):,}")

    ops = collections.Counter(); conf = collections.Counter()
    n_new = n_ambig = n_matched = 0
    review = []
    sql = ["-- assertions generated by ades_overlay_loader.py",
           f"-- source_batch={a.source_batch}  window={a.time_window}s  radius={a.max_radius}\"  ratio={a.ratio}"]
    cache = {}
    with connect(host=a.host) as conn:
        ...
        for (stn, trk), obss in tracklets.items():
            ...
            if orig_trks:
                ...
                orig_obsids = list(sn['obsid'][mask])
                upds = [sn['upd'][i] for i in np.where(mask)[0]]
                basis = max(upds) if upds else None
                orig_lit = "ARRAY[" + ",".join("'" + o + "'" for o in orig_obsids) + "]"
                basis_lit = "'" + basis.isoformat() + "'" if basis else "NULL"
                sql.append(f"INSERT INTO css_ades_overlay.tracklet(overlay_trk_id,source_batch,stn,new_trksub,op,original_obsids,match_confidence,basis_max_updated) "
                           f"VALUES ('{oid}','{a.source_batch}','{stn}','{trk}','supersede',{orig_lit},'{mc}',{basis_lit});")
            else:
                sql.append(f"INSERT INTO css_ades_overlay.tracklet(overlay_trk_id,source_batch,stn,new_trksub,op,original_obsids,match_confidence) "
                           f"VALUES ('{oid}','{a.source_batch}','{stn}','{trk}','add',ARRAY[]::text[],'{mc}');")
            for o in obss:
                vals = ",".join(sql_lit(o.get(af), col) for col, af in COLMAP)
                sql.append(f"INSERT INTO css_ades_overlay.obs(overlay_trk_id,{','.join(c for c,_ in COLMAP)}) VALUES ('{oid}',{vals});")
    open(out, "w").write("\n".join(sql) + "\n")

    print(f"\nReconciliation ({time.time()-t0:.0f}s):")
    ...
    print(f"\nSQL assertions written to {out}  (apply as the schema owner)")
